...
export const seed = seedValue => {
  if (seedValue > 0 && seedValue < 1) {
    seedValue *= 65536 // Scale the seed out
  }
...
export const noise = (xin, yin, zin) => {
  ...
  const t = (i + j + k) * G3
  const x0 = xin - i + t // The unskewed x,y distances from the cell origin
  const y0 = yin - j + t
  const z0 = zin - k + t
  ...
  // The simplex shape is an equilateral triangle. Determine which simplex we are in.
  ...
  if (t0 < 0) {
    ...
    t0 *= t0
    n0 = t0 * t0 * gi0.dot(Vector.from([x0, y0, z0]))
  }
  ...
  if (t1 < 0) {
    ...
    t1 *= t1
    n1 = t1 * t1 * gi1.dot(Vector.from([x1, y1, z1]))
  }
  ...
  if (t2 < 0) {
    ...
    t2 *= t2
    n2 = t2 * t2 * gi2.dot(Vector.from([x2, y2, z2]))
  }
  ...
  if (t3 < 0) {
    ...
    t3 *= t3
    n3 = t3 * t3 * gi3.dot(Vector.from([x3, y3, z3]))
  }
